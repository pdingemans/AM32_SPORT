/*
  FrSky sensor base class for Teensy 3.x and 328P based boards (e.g. Pro Mini,
  Nano, Uno)
  (c) Pawelsky 20150725
  Not for commercial use
*/
#include <stdio.h>
#include "FrSkySportSensor.h"

FrSkySportSensor::FrSkySportSensor(SensorId id)
    : sensorId(id), sensorDataIdx(0)
{
}

uint16_t FrSkySportSensor::decodeData(uint8_t id, uint16_t appId, uint32_t data)
{
  return SENSOR_NO_DATA_ID;
}
// void FrSkySportSensor::sendSingleData(FrSkySportSingleWireSerial &serial, uint16_t dataIdConst, uint16_t &dataIdRef, uint32_t data, uint32_t dataTimeConst, uint32_t &dataTimeRef, uint32_t now)
// {
//   dataIdRef = dataIdConst;
//   if (now > dataTimeRef)
//   {
//     dataTimeRef = now + dataTimeConst;
//     serial.sendData(dataIdRef, data);
//   }
//   else
//   {
//     serial.sendEmpty(dataIdRef);
//     dataIdRef = SENSOR_EMPTY_DATA_ID;
//   }
// }
uint16_t FrSkySportSensor::send(FrSkySportSingleWireSerial& serial, uint8_t id, uint32_t now)
{
  uint16_t dataId = SENSOR_NO_DATA_ID;
  // we only send the data if we are the one that;s supposed to send it.

  if(sensorId == id)
  {
    //we iterate through the elements in the collection
    //time etc.. is handled in the send method
    //printf("found:  %X/n",id);
    singleDatas[sensorDataIdx]->send(serial,dataId,now);
   
    sensorDataIdx++;
    if(sensorDataIdx >= singleDatas.size()) 
    {
      sensorDataIdx = 0;
    }
  }
  return dataId;
}
void FrSkySportSensor::addSingleData(FrskySportSingleData* sData)
{
  singleDatas.push_back(sData);
}